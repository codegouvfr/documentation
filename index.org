...
D’abord, pour l’administration publique, la mission logiciels libres
recense des logiciels libres sur trois pages web :
- [[https://code.gouv.fr/sill][code.gouv.fr/sill]] permet d’identifier facilement quels sont les
  logiciels libres utilisés par l’adiministration, leur version
  utilisée, et d’avoir un référent du logiciel à contacter pour avoir
  plus d’informations. Nous vous recommandons de commencer par là.
...
- [[https://code.gouv.fr/sources][code.gouv.fr/sources]] liste de manière centralisée les dépôts de codes
  ...
  et trier les données en fonction de multiples critères.
...
De façon plus gérénale, il existe plusieurs consortiums et ensembles
...
la licence) et anisi potentiellement contribuer à son amélioration pour
le bénéfice de tous.

Cette liberté a des répercussions économiques bénéfiques puisque
n’importe qu’elle peut apporter du support au logiciel, et chaque
entreprise ou individu apportant leurs contributions mutualisent la
charge de travail.

La réponse est donc un grand *oui* ! Il est tout à fait possible de
faire évoluer une solution libre. Pour savoir comment le faire, vous
pouvez vous référer [[*Trouver des solutions libres][sections ci-dessus.]]



** Se répérer dans l’écosytème logiciel libre et open source

*** Dans l’administration

*** Hors administration

** Quelle attention au modèle économique ?
...
p** Bonnes pratiques de publication
...
** Pour une startup d’État

** Pour un projet d’intérêt général sensible
...
Néanmoins, si l’erreur a été faite, ou que le projet était d’abord
fermé et cherche maintenant à s’ouvrir, il existe certains outils :
...
- [[https://github.com/GitGuardian][Gitguardian]] sous MIT
...
La principale différence entre GitHub et Gitlab se trouve dans la licence et le
modèle économique.

GitHub est un logiciel propriétaire ; son code n’est ni visible, ni
modifiable. GitLab est un logiciel tout au plus open source, sous la licence
MIT, ou, a minima, /source available/. Cela signife que l’on peut voir et
étudier le code source, mais on ne peut pas le réutiliser librement.

GitHub a un modèle très classique : c’est une platforme basée sur un logiciel
propriétaire. GitLab a un modèle dit /open core/ : une version du logiciel
communautaire open source ([[https://gitlab.com/rluna-gitlab/gitlab-ce][GitLab CE]]), et une version plus complète avec des
fonctionnalités supplémentaires propriétaires payantes disponible sous une
