...
Elle s’adresse à plusieurs audience :

- Les équipes métier et chefs de projet qui souhaitent publier code
  source de leurs projets et applications sous licence libre
- Les DSI pour l’utilisation des logiciels libres dans leur SI
- Aux directeurs d’aministration et les équipes métier prêtes à
...
D’abord, pour les administrations publiques, la mission logiciels libres
recense des logiciels libres sur trois pages web :
...
beta.gouv.fr a aussi un outil appelé [[https://etabli.incubateur.net/][établi]] répertoriant les logiciels de
l’administration publique avec une fiche descriptive orientée métier
...
d’affiner ses recherches pour être sûr de ne pas louper un logiciel à
potentiel de réutilisation. *Attention*, cet outil référence tant des
logiciels libres que des logiciels propriétaires.

Ensuite, l’utilisation des moteurs de recherches classiques font
généralement l’affaire. Avec l’utilisation des mots clefs « open source »
et « logiciel libre », les résultats sont satisfaisants. Néanmoins, des
recherches plus abouties sont nécessaires : aller sur le site web
principal du logiciel, vérifier si la licence est indiqué ; mieux
encore, trouver le dépôt de code source du logiciel et vérifier à la
source quelle licence est apposée sur le logiciel.

Enfin, il est aussi possible d’aller voir sur des platformes telles que
github.com, gitlab.com, sr.ht (sourcehut), mais aussi des platformes
spécialisées pour un langage comme [[https://pypi.org/][pypi.org]] (Python) ou [[https://www.npmjs.com/][npmjs.com]]
(JavaScript) pour trouver des modules logiciels.

!> *Attention*, la présence d’un dépôt logiciel sur ces platformes
...
Le site mentionné plus haut ([[https://code.gouv.fr/sill][code.gouv.fr/sill]]) est un excellent point
de départ pour les adiministrations. Non seulement vous pouvez contacter
le référent ou la référente du logiciel libre qui, de fait, le connaît
et l’utilise dans son administration, mais le site recense aussi des
prestataires sur le logiciel en particulier capables de vous fournir de
l’expertise.

[[https://code.gouv.fr/fr/utiliser/marches-interministeriels-support-expertise-logiciels-libres/][Les marchés interministériels support et exprite à l’usage des logiciels
...
libres. On notera de façon non-exhaustive :
...
Il est donc *tout à fait possible de faire évoluer une solution libre*. Pour
savoir comment le faire, vous pouvez vous référer [[*Trouver des solutions libres][sections ci-dessus.]]

** Les politiques ministérielles déjà publiées

Vous pouvez lire notre [[https://code.gouv.fr/fr/blog/lengagement-des-ministeres-sur-louverture-des-codes-sources-et-lutilisation-de-logiciels-libres-retour-sur-les-feuilles-de-route-publiees-en-septembre-2021/][entrée de blog sur le sujet]]. Pour résumé, voici
...
début. L’une des raisons principales et que, par philosophie, les
...
tend à être très décentralisé, sans point unique où trouver toute les
communautés, tous les logiciels, et toutes les ressources.

Cela étant dit, pour se repérer dans l’écosystème un certains nombres de
...
est vivement animé par des associations et entreprises du libres.
...
- [[HTTPS://www.linuxfoundation.org/][Linux Foundation]], un consortium à but non lucratif visant à protéger
  et standardiser le noyaux Linux en procurant les ressources pour
...
Enfin, il y aussi un écosystème d’entreprises du libre dont nous avons
parlé dans [[*Trouver de l'expertise sur une solution libre][cette section]], et pour lesquelles nous allons nous pencher sur
leur modèles économiques dans la section suivante.
...
être redistribuer gratuitement par le premier acheteur, ont donc dû
...
différents besoins, avec chacune des modèles différents, qui ne sont
...
L’un des modèles les plus évident pour valoriser des logiciels libres en
...
communautaire existant et accompagne ses clients dans le déploiements du
...
Le modèle /Open Core/ consiste a éditer un logiciel de base sous licence
libre (souvent appeler la « version communautaire ») et vendre des
extensions propriétaires, ou vendre des outils de développement
propriétaires au dessus du logiciel.
...
Un modèle a double licence signifie qu’une solution logiciel est
...
1. Un scénario où l’utilisateur final peut choisir l’une ou l’autre
   licence. L’idée est de prendre une licence de type copyleft avec
   beaucoup de restrictions afin que les utilisateurs commerciaux se
   reportent vers la licence commerciale payante pour soit ne pas
   s’embarrasser avec les problèmes de conformités légales de la licence
   copyleft dans une solution intégrée propriétaire, soit développer sur
   le logiciel sans avoir à dévoiler leurs contributions.

2. Un scénario où plusieurs licences s’appliquent en même temps sur le
   logiciel. Cela arrive surtout lorsque certaines briques logiciels
   sont utilisées et leurs licences sont peu compatibles. Ce n’est pas
   par essence un modèle économique.

Il est aussi possible qu’une solution logiciel ne soit pas sous double
licence par défaut, mais qu’il y est un changement au court du
temps. Par exemple :
...
2. Une licence propriétaire comportant une clause de réversibilité en
   licence libre si, par exemple, l’entreprise est amené à
   disparaître.
...
conseil, de formations. Pour avoir des revenus récurrents, une
entreprise peut facture du support forfaitaire, des garanties de
...
majorité des cas, un DCO ou CLA.
...
redistribuer). Cela permet au projet de se protéger contre de potentiel
attaque en justice en lien avec le droit d’auteur des contributions.
...
Par conséquent la fondation Linux, et plusieurs autres organisations qui
ont suivi, sont passées au *DCO*, /[[https://developercertificate.org/][Developer Certificate of
...
Tchap) [[https://element.io/blog/synapse-now-lives-at-github-com-element-hq-synapse/][font signer un CLA avec une exception à la AGPL pour pouvoir
vendre du code source]] contribué (par des auteurs extérieurs à Element)
sous une licence propriétaire.

*** Publication

S’agissant d’une administration publique qui publie des codes sources
sous licences libres, il faut bien comprendre que les libertés octroyés
par ces licences permettent en tout temps à n’importe quel acteur de
réutiliser le code produits par des agents public et ce à même à des
fins lucratives.

Si les codes sources sont estimés représenter une menace avérée pour
l’intérêt général, alors un choix de licence à copyleft fort est
fortement conseillé. Pour plus de détails sur le copyleft fort, [[la section ][se
référer à cette section]]. Attention, le copyleft fort n’empêche pas la
...
/Openwashing/, dérivé du /greenwashing/ (et tout les autres mots-valises
en -/washing/). Le mot /[[https://www.fauxpensource.org/][fauxpen]]/ signifie la même chose :
...
Trois questions fondamentales à se poser pour savoir si c’est un projet
libre :

!> Est-ce que la licence garantie les [[https://www.gnu.org/philosophy/free-sw.en.html#four-freedoms][quatre libertés fondamentales]] ou
répond aux critères de la [[http://www.opensource.org/osd.html][définition de l’OSI]] ?

Pour vous faciliter la vie, l’OSI maintient une [[http://www.opensource.org/licenses][liste de licences
acceptées]].

[[https://github.com/redis/redis?tab=License-1-ov-file][Redis]] et [[https://github.com/hashicorp/terraform?tab=License-1-ov-file][Terraform]] sont des exemples.

*** Des /success stories/

Les /success stories/ dans le privée sont désormais compliquées à
...
Pour le public on peut citer : [[https://lutece.paris.fr/fr/][Lutece de la ville de Paris]], [[https://www.schleswig-holstein.de/DE/landesregierung/ministerien-behoerden/I/Presse/PI/2024/CdS/240403_cds_it-arbeitsplatz.html][un état
fédéral allemand fait passer 30 000 PC sous Linux et LibreOffice]],
[[https://www.ccomptes.fr/sites/default/files/2024-07/20240710-S-2024-0754-Pilotage-transformation-numerique-Etat-par-direction-interministerielle-du-numerique.pdf][Lesystème de design de l’État (DSFR) permet entre 3,1 et 4,9 M€
d’économies par an (note de bas page 86)]]. Vous pouvez voir une liste
plus complète sur [[https://code.gouv.fr/awesome][code.gouv.fr/awesome]].
